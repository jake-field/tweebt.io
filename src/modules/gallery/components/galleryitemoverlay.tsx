...
export default function GalleryItemOverlay({ item, visible, showMetrics, showTweetText, showAuthors, children }: Props) {
		...
		<>
			...
			<div className={`${(topAndBottomLayout && !mobilemode) ? 'absolute' : ''} w-full h-full flex flex-col text-xs pointer-events-none text-white`}>
				<div className={`pointer-events-auto z-10 transition-all cursor-default relative ease-in-out duration-150 ${visible ? 'top-0' : '-top-20'}`}>
					{showAuthors && <div className={`bg-black bg-opacity-70 backdrop-blur-sm gap-2 justify-center items-center py-2 px-1 flex flex-wrap`}>
						<Link href={`/@${item.author.username}`}>
							<a title={`View ${item.author.name}'s (@${item.author.username}) Profile`} className={linkClassName + ' flex flex-wrap items-center justify-center gap-1'}>{item.author.name}{!item.referencing ? <span className='text-gray-300 font-light'>@{item.author.username}</span> : ''}</a>
						</Link>
