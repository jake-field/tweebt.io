...
export default function GalleryItemOverlay({ item, visible, showMetrics, showTweetText, children, topAndBottomLayout, mobilemode }: Props) {
	...
	useEffect(() => {
		if (item.text) setFormattedText(FormatTwitterText(item.text, true));
	...
	return (
		<>
			...
			<div className={`${(topAndBottomLayout && !mobilemode) ? 'absolute' : ''} w-full h-full flex flex-col text-xs pointer-events-none text-white`}>
				<div className={`${bannerClass} z-10 gap-2 justify-center flex-wrap ${visible ? 'top-0' : '-top-20'}`}>
					...
					{item.text && item.text.length > 0 && showTweetText &&
						<p className={`p-1 overflow-hidden pb-0 w-full border-t border-gray-900 border-opacity-70 transition-all ease-in-out duration-300 ${visible ? 'h-max' : 'h-0'}`}>
							{formattedText}
						</p>
					}
				</div>
