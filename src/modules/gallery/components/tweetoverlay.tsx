...
export default function TweetOverlay({ item, visible, hidePoster, showMetrics, showTweetText, mobilemode }: Props) {
	return (
		<div className={`${styles.banneroverlay} ${(!mobilemode) ? 'absolute' : ''} ${visible ? 'opacity-100' : 'opacity-0'}`}>
			{(!hidePoster || (hidePoster && item.ref_tweet)) &&
						...
						<>
							...
							{item.ref_tweet.author.handle !== item.tweet.author.handle &&
								<Link
									...
									title={`View ${item.ref_tweet.author.name === '[Deleted Tweet]' ? item.ref_tweet.author.handle : item.ref_tweet.author.name}'s (@${item.ref_tweet.author.handle}) Profile`}
								>
									{item.ref_tweet.author.name !== '[Deleted Tweet]' && <Image src={item.ref_tweet.author.image} alt={item.ref_tweet.author.name + '\'s profile image'} width={24} height={24} />}
									{!hidePoster ? (
										<>
										...
										<>
											{item.ref_tweet.author.name || item.ref_tweet.author.handle}
											<span className='text-gray-300 font-light'>@{item.ref_tweet.author.handle}</span>
										</>
									)}
								</Link>
							}
